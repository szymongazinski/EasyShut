...
if (-not $TestBuild) {
    ...
    & $compiler @setupFlags /define:UNINSTALL "/out:$out\EasyShut-uninstall.exe" "$repo\src\Setup.cs" "$repo\src\AssemblyInfo.cs"
    ...
    & $compiler @setupFlags "/out:$out\EasyShut-Setup.exe" @resources "$repo\src\Setup.cs" "$repo\src\AssemblyInfo.cs"

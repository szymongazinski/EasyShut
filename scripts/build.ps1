...
$gui = @('/target:winexe', "/out:$out\EasyShut-window.exe", '/reference:System.Windows.Forms.dll', '/reference:System.Drawing.dll', "/resource:$repo\assets\EasyShut.ico,EasyShut.App.ico") + $sources + @("$repo\src\Gui.cs", "$repo\src\HelpWindow.cs", "$repo\src\AdvancedWindow.cs", "$repo\src\AppIcon.cs")
...
if (-not $TestBuild) {
    ...
    & $compiler @setupFlags /define:UNINSTALL "/out:$out\EasyShut-uninstall.exe" "$repo\src\Setup.cs" "$repo\src\AssemblyInfo.cs" "$repo\src\AppIcon.cs"
    ...
    & $compiler @setupFlags "/out:$out\EasyShut-Setup.exe" @resources "$repo\src\Setup.cs" "$repo\src\AssemblyInfo.cs" "$repo\src\AppIcon.cs"
